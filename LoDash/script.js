...
// (Task 1) Output array of skills of all people, with no repeats and sorted alphabetically
var skillsList = [];
var uniqSkills = [];
var skills = _.map(data, function(key) {
        _.filter(key.skills, function(item){
            skillsList.push(item.toLowerCase());
            uniqSkills = _.sortBy((_.uniq(skillsList)), function(sort){
                return sort.charCodeAt();
        });
    });
});
console.log('Uniq skills sort by alphabet', uniqSkills);

// (Task 2) Output array of names of people sorted depending on the number of friends
var nameSort =_.sortBy(data, 'friends');
var names = _.map(nameSort, 'name');
console.log('Peoples names', names);

// (Task 3) Output array of all the friends of all users, with no repeats
var friendsList = _.map(data, 'friends');
var friendsNameList = [];
var uniqFriends = [];
_.forOwn(friendsList, function(key) {
    _.filter(key, function(item){
        friendsNameList.push(item.name);
        uniqFriends = _.uniq(friendsNameList);
    });
});
